...
export function exportToPDF(title, headers, data, fileName = 'تقرير_حضور') {
  ...
  const htmlContent = `
    ...
    <head>
      ...
      <script>
        // تشغيل الطباعة التلقائية فور اكتمال تحميل الصفحة والخطوط
  ...
  // التحقق مما إذا كان المستخدم يتصفح من هاتف ذكي (iOS / Android) لتفادي مشكلة طباعة الـ iframe في Safari
  ...
  if (isMobile) {
    ...
    if (printWindow) {
      ...
      alert('الرجاء السماح بالنوافذ المنبثقة (Pop-ups) لطباعة وحفظ التقرير.');
    }
  } else {
    // على أجهزة الكمبيوتر، نستخدم طريقة الـ iframe المخفي لتجربة طباعة فورية وسلسة
